...
export default function AICoachRag({ goal, onGoalChange, material, onMaterialChange }) {
  ...
  const handleUpload = (f) => {
    ...
    reader.onload = () => {
      ...
      onMaterialChange && onMaterialChange({ id: uid('mat'), name: f.name, text, segments: segs })
    }
    reader.readAsText(f)
  }
  ...
  const markComplete = (segId, done) => {
    ...
    onMaterialChange && onMaterialChange(next)
  }
  ...
  const ask = async (question) => {
    ...
    if (strong.length === 0) {
      setQa((prev) => [...prev, { id: uid('qa'), role: 'assistant', text: '해당 질문에 답하기 위한 근거를 자료에서 찾지 못했어요. 관련 내용을 포함한 파일을 추가로 업로드해 주세요.' }])
      setQPending(false)
      return
    }
    ...
    const answer = [
      ...
      '핵심 근거:',
      ...bullets,
      '\nTIP: 목표/마감을 조정하고 부족 카테고리에 집중 세션을 생성하세요.',
    ].join('\n')
      ...
      <div className="p-5 border-b">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          <div>
            ...
            <input value={goal?.description || ''} onChange={(e)=> onGoalChange && onGoalChange({ ...goal, description: e.target.value })} className="mt-1 w-full border rounded-lg px-3 py-2" placeholder="예: 부정사·동명사 100% 완주" />
          </div>
          <div>
            <label className="text-xs text-slate-600">마감일</label>
            <input type="date" value={goal?.deadline || ''} onChange={(e)=> onGoalChange && onGoalChange({ ...goal, deadline: e.target.value })} className="mt-1 w-full border rounded-lg px-3 py-2" />
          </div>
          <div className="flex items-end gap-2">
            {summary && (
              <div className="text-sm text-slate-700 bg-slate-50 rounded-lg px-3 py-2 flex-1">
                ...
                {dailyTarget !== undefined && <span className="ml-2">· 일일 목표 <b>{dailyTarget}분</b></span>}
              </div>
            )}
          </div>
        </div>
        ...
        {activeTab === 'Library' && (
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="lg:col-span-4 border rounded-2xl p-4">
              ...
              <div className="mt-3 text-xs text-slate-600">텍스트/마크다운 기반 데모입니다.</div>
              <div className="mt-4 p-3 rounded-xl bg-slate-50 text-sm">
                ...
                <div className="text-slate-600">세그먼트 {material?.segments?.length || 0}개</div>
              </div>
            </div>
        ...
        {activeTab === 'Tracker' && (
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="lg:col-span-4 border rounded-2xl p-4">
              ...
              {dailyTarget !== undefined && (<div className="mt-2 text-xs text-slate-600">마감까지 일일 목표: <b>{dailyTarget}분</b></div>)}
            </div>
            <div className="lg:col-span-8 border rounded-2xl p-4">
        ...
        {activeTab === 'Chat' && (
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="lg:col-span-5 border rounded-2xl p-4">
              ...
              <div className="mt-3 text-xs text-slate-600">※ 업로드 자료의 문장 일부만을 증거로 사용합니다. 근거가 없으면 답변을 거절합니다.</div>
            </div>
          ...
          <div key={m.id} className={`my-2 ${m.role==='user'?'text-right':'text-left'}`}>
            ...
            {m.citations && m.citations.length>0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {m.citations.map((c,i)=>(<span key={i} className="text-[11px] px-2 py-1 rounded-full bg-white border">근거 {i+1}: {c.title}</span>))}
              </div>
            )}
          </div>
        ))}
        {pending && (<div className="my-2 inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-slate-100 text-slate-800 text-sm"><Loader2 className="w-4 h-4 animate-spin"/>생각 중…</div>)}
        <div ref={endRef} />
      </div>
